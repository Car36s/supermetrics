...
describe('lib/senders', () => {
  describe('getSendersWithPostCounts', () => {
    it('should parse senders from posts', () => {
      expect(getSendersWithPostCounts(posts)).toStrictEqual(sendersWithPostCounts)
    })
  })
  ...
  describe('getFilteredSenders', () => {
    it('should filter ', () => {
      expect(getFilteredSenders(sortedSenders, posts[2].from_name)).toStrictEqual([sortedSenders[0]])
    })
  })
})
